...
function SmallActionButton({ variant = "neutral", children, onClick, title }) {
  const base =
    "inline-flex items-center gap-2 text-xs px-3 py-2 rounded-lg border transition font-medium shadow-sm";
  const styles =
    variant === "danger"
  ...
  return (
    <button type="button" onClick={onClick} title={title} className={cx(base, styles)}>
      {children}
    </button>
  );
}
...
export default function AdminAnalytics() {
  ...
  const recentSessions = useMemo(
    () => computeRecentSessions(events, 20),
    [events]
  );
      ...
      <div className="px-6 space-y-6">
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <KpiCard
              ...
              sub="per device/tab over time"
            />
            <KpiCard
              title="Avg time/session"
              value={msToHuman(overview.avgSessionMs)}
              sub="based on start/end events"
            />
            <KpiCard
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <SectionCard
            ...
            subtitle="Ranked by section_view events"
          >
            {sectionViewsRanked.length ? (
              <>
                <SimpleBarChart
                  data={sectionViewsRanked.map((d) => ({
                    section: d.section,
                    value: d.value,
                  }))}
                />
          ...
          <SectionCard
            title="Time spent by section"
            subtitle="Sum of section_time durations"
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <SmallActionButton
                ...
                onClick={() => {
                  if (
                    !window.confirm(
                      "Clear ALL local analytics on this browser?"
                    )
                  )
                    return;
                  resetAnalytics();
