...
/**
 * Code Lab deep-link mapping.
 *
 * This is Platform presentation/navigation behavior,
 * not Profile Variant content.
 */
const CODE_SNIPPETS_FROM_BY_MATCH = [
...

  {
    ...
    from:
      "syzmaniac,sys_managed",
  },
];


function getCodeLabFrom(
  projectName
) {
  const name =
    String(
  ...
  const hit =
    CODE_SNIPPETS_FROM_BY_MATCH.find(
      (entry) =>
        name.includes(
          entry.match
        )
